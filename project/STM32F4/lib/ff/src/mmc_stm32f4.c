...
)
{
	...
	init_int_timer();					/* set and start interrupt timer for timerproc*/
...
/**************************************************************************/
/*!
    @brief  to set interrupt Timer.	each 10 msec, interrupt occur. 				@n
	@param  none
    @retval : none
*/
/**************************************************************************/
void init_int_timer(void){
	...
	/* Compute the prescaler value. TIM7 is APB1 = 48MHz * 2 */
	int PrescalerValue = (uint16_t)((SystemCoreClock / 2 / 10000)) - 1;
